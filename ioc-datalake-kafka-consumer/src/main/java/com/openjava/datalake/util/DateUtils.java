...
public class DateUtils {
    ...
    public static String getTimestamp() {
        return formatDate(new Date(), DateUtils.TIMESTAMP_FORMAT);
    }
    ...
    /**
     * 获取本周开始日期
     * @param isFirstDayOfWeekIsMonday 指定一周的开始（默认false 周日  true 周一）
     * @return
     */
    public static Date getCurrentWeekBeginDate(Boolean isFirstDayOfWeekIsMonday) {
    	Calendar calendar = Calendar.getInstance();
    	if (isFirstDayOfWeekIsMonday != null && isFirstDayOfWeekIsMonday) {
    	...
    	if (dayOfWeek > firstDayOfWeek) {
			...
			calendar.add(Calendar.DATE, firstDayOfWeek-dayOfWeek-7);
		}
     ...
     * 获取指定日期的上周开始日期  
     * @param isFirstDayOfWeekIsMonday 指定一周的开始（默认false 周日  true 周一）
     * @param date 指定的日期  默认当前时间
     * @return
     */
    public static Date getLastWeekBeginDate(Boolean isFirstDayOfWeekIsMonday, Date date) {
    	Calendar calendar = Calendar.getInstance();
    	if (date != null) {
			calendar.setTime(date);
		}
     ...
     * @return
     */
    public static Date getLastWeekEndDate(Boolean isFirstDayOfWeekIsMonday, Date date) {
    	Calendar calendar = Calendar.getInstance();
    	if (date != null) {
			calendar.setTime(date);
		}
    	...
    	calendar.set(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH), 0, 0, 0);
    	int dayOfWeek = calendar.get(Calendar.DAY_OF_WEEK);
    	calendar.add(Calendar.DATE, firstDayOfWeek-dayOfWeek-1);
    	return calendar.getTime();
    }
    
    /**
     * 取得某个时间前n天,返回日期格式为yyyy-MM-dd
     * @param date
     * @param n
     * @return
     */
 	public static String getNDayBeforeOneDate(Date date, int n) {
 		Calendar c = Calendar.getInstance();
 		c.setTime(date);
 		c.add(Calendar.DAY_OF_MONTH, -n);
 		int m = c.get(Calendar.MONTH) + 1;
 		int d = c.get(Calendar.DATE);
 		return "" + c.get(Calendar.YEAR) + (m < 10 ? "-0" : "-") + m
 				+ (d < 10 ? "-0" : "-") + d;
 	}
 	
 	/**
 	 * 获取指定日期前N周对应的周结束日期集合  格式yyyy-MM-dd
 	 * @param date 指定的日期 默认当天
 	 * @param weeks 返回指定日期往前的N天周 含指定日期
 	 * @param isFirstDayOfWeekIsMonday 指定一周的开始（默认false 周日  true 周一）
 	 * @return
 	 */
 	public static List<String> getNDateBeforeForWeeks(Date date, int weeks, Boolean isFirstDayOfWeekIsMonday) {
 		Calendar calendar = Calendar.getInstance();
 		if (date != null) {
			calendar.setTime(date);
		}
 		Date lastWeekEndDate = DateUtils.getLastWeekEndDate(isFirstDayOfWeekIsMonday, calendar.getTime());
    	List<String> weeksList = new ArrayList<String>();
    	weeksList.add(getDate(calendar.getTime(), "yyyy-MM-dd"));
    	calendar.setTime(lastWeekEndDate);
 	...
 	/**
 	 * 获取指定日期前N天对应的日期集合  格式yyyy-MM-dd
 	 * @param date 指定的日期 默认当天
 	 * @param days 返回指定日期往前的N天   含指定日期
 	 * @return
 	 */
 	public static List<String> getNDateBeforeForDays(Date date, int days) {
 		Calendar calendar = Calendar.getInstance();
 		if (date != null) {
 			calendar.setTime(date);
 		}
 		...
 		for (int i = 0; i < days; i++) {
 			daysList.add(getDate(calendar.getTime(), "yyyy-MM-dd"));
 	...
 	/**
	 * 获取指定年、月、日 的最大时间
	 * @param dateStr 传入指定时间  格式  例如：yyyy-MM-dd
	 * @param returnDateFormat 返回时间格式  默认yyyy-MM-dd HH:mm:ss
	 * @param n 获取传入时间前或后n个单位的时间
	 * @return 指定年、月、日 的最大时间
	 */
	public static String getMaxDateTime(String dateStr, String returnDateFormat, Integer n) {
	    if (returnDateFormat == null || "".equals(returnDateFormat)) {
            returnDateFormat = "yyyy-MM-dd HH:mm:ss";
        }
        SimpleDateFormat sdf = new SimpleDateFormat(returnDateFormat);
        
	    String[] split = dateStr.trim().split("-");
        Date endDate = null;
        ...
        if (length == 1) {//年
            ...
            c.set(year, 11, c.getActualMaximum(Calendar.DAY_OF_MONTH), 23, 59, 59);
            if (n != null) {
            ...
            c.set(year, month - 1, maxDay, 23, 59, 59);
            if (n != null) {
                c.add(Calendar.MONTH, n);
            }
            endDate = c.getTime();
        }else if (length == 3) {//年月日
            year=Integer.valueOf(split[0]);
            month=Integer.valueOf(split[1]);
    ...
    public static void main(String[] args) {
    	String maxDateTime = DateUtils.getMaxDateTime(DateUtils.getDate("yyyy"), "yyyy-MM-dd", -1);
