...
import com.openjava.datalake.rescata.dto.DlRescataTagRelationDTO;
...
public class DlRescataResource implements Serializable, Cloneable {
    ...
    @ApiModelProperty("删除标识(0正常1删除 用于资源目录逻辑删除)")
    @Column(name = "IS_DELETE")
    private Long isDelete;

    @ApiModelProperty("（dl.resource.open.society）0不向社会公开;1向社会公开")
    @Column(name = "IS_OPEN_SOCIETY")
    private Long isOpenSociety;

    @ApiModelProperty("（dl.resource.mount）0未挂载;1已挂载")
    @Column(name = "IS_MOUNT")
    private Long isMount;

    @ApiModelProperty("归属库ID(类ID)")
    ...
    @ApiModelProperty("受理人科室ID")
    @Column(name = "ASSIGNEE_SECTION_ID")
    private String assigneeSectionId;

    @ApiModelProperty("受理人科室CODE")
    @Column(name = "ASSIGNEE_SECTION_CODE")
    private String assigneeSectionCode;

    @ApiModelProperty("统一机构代码")
    ...
    @ApiModelProperty("公开条件")
    @Column(name = "OPEN_CONDITION")
    private String openCondition;

    @ApiModelProperty("关键字集合String")
    @Column(name = "KEYWORD_ARR")
    ...
    @Transient
    @ApiModelProperty("模板文件是否修改(否:0,是:1)")
    private Long isUpdateFile;

    @Transient
    @ApiModelProperty("分类树级名称字符串集合")
    private List<String> classNameString;

    @Transient
    @ApiModelProperty("库表挂载-数仓主题分类ID")
    private Long typeId;

    @Transient
    @ApiModelProperty("标签集合")
    private List<DlRescataTagRelationDTO> tagList;
